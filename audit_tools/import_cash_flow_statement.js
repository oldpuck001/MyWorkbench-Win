// import_cash_flow_statement.js

const { ipcRenderer } = require('electron');

let file_path = '';

export async function import_cash_flow_statement() {
    ...
    document.getElementById('selectButton').addEventListener('click', async () => {
        ...
        // 將文件路徑發送到主進程
        ipcRenderer.send('asynchronous-message', { command: 'select_cash_flow_statement', data: {'file_path': file_path }});
    });
    ...
    document.getElementById('importButton').addEventListener('click', async () => {
        ...
        ipcRenderer.send('asynchronous-message', { command: 'import_cash_flow_statement', data: data });
    });
    ...
    ipcRenderer.on('asynchronous-reply', (event, result) => {
        ...
        if (result[0] === 'select_cash_flow_statement') {
            // 将文件路径显示在输入框中
        ...
        if (result[0] === 'import_cash_flow_statement') {
